...
def get_toast_lnX_loesssmooth((dfile, mfile, span)):
    d = np.load(dfile)
    d = loess2d(d, span)
    m = np.load(mfile)
    d = ma.compressed(ma.masked_array(d, mask=m))
...
def write_toast_lnX(ddir, dmeas, meas, root, w, srcs, dtype, fstring, 
                    msmaskdir, msmaskstr, span, outputfile):
    args = ((os.path.join(ddir, dmeas, fstring.format(root, meas, w, s, dtype)), 
            os.path.join(msmaskdir, msmaskstr.format(root, s)),
            span) for s in srcs)
    pool = mp.Pool(mp.cpu_count() - 1)
    res = pool.map_async(get_toast_lnX_loesssmooth, args)
    res = res.get()
    # get all the arrays in a single line
    res = np.hstack(res)
    bp.io.write_fem(outputfile, res)

def write_toast_phi(ddir, dmeas, meas, root, w, srcs, dtype, fstring, 
                    msmaskdir, msmaskstr, span, outputfile, rpath=None, rfile=None):
    if rpath is None:
        args = ((os.path.join(ddir, dmeas, fstring.format(root, meas, w, s, dtype)), 
                os.path.join(msmaskdir, msmaskstr.format(root, s)),
    ...
    pool = mp.Pool(mp.cpu_count() - 1)
    res = pool.map_async(get_toast_phi_loesssmooth, args)
    res = res.get()
    # get all the arrays in a single line
    res = np.hstack(res)
    bp.io.write_fem(outputfile, res)
...
def sample_data(ddir, tdir, rdir, tmeas, rmeas, root, s_nrow, s_ncol, wls, nrow, ncol, 
                 ssmaskdir, musp_corr, span, ro=0, co=0, rs=5, cs=5,
                 sro=0, sco=0, srs=1, scs=1, srcs_exclude=None):
    ...
    Args:
        ...
        root: study name (eg 20130410_G3033_Patient)
    ...
    print >>sys.stderr, 'root =', root

    # some variables
    df_string = '{0}_{1}_wl{2}_s{3}_{4}.npy'
    ssmask_string = '{0}_wl{1}_s{2}_mask.npy'
    msmask_dir = 'masks_ms'
    ...
    # exclude sources
    if srcs_exclude is not None:
        srcs = np.setdiff1d(srcs, srcs_exclude)
    ...
    for s in srcs:
        ...
        for w in wls:
            mask = np.load(os.path.join(ssmaskdir, ssmask_string.format(root, w, s)))
            src_mask |= mask 
        
        # pick out only those on the sample grid and save it to disk
        src_mask |= g
        np.save(os.path.join(msmask_dir, msmask_string.format(root, s)), src_mask) 
        # save them on the master mask
        master_mask &= src_mask
    # save master grid on disk
    np.save(os.path.join(msmask_dir, msmask_master_string.format(root)), master_mask)
    ...
    if srcs_exclude is not None:
        toastddir = 'toastdata_{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}_exclude_srcs'.format(sro, sco, srs, scs, ro, co, rs, cs)
    if not os.path.exists(toastddir):
        os.makedirs(toastddir)
    
    # QM file
    qmfile = os.path.join(toastddir, 
                          '{0}_{1}_{2}_{3}_{4}.qm'.format(root, ro, co, rs, cs))
    ...
    for s in srcs:
        m = np.load(os.path.join(msmask_dir, msmask_string.format(root, s)))
    ...
    for w in wls:
        ...
        write_toast_lnX(ddir, tdir, tmeas, root, w, srcs, 'DC', 
                            df_string, msmask_dir, msmask_string, span, outputfile)
        # D ref
        outputfile = os.path.join(toastddir, fem_str.format('ref', 'lnD', w))
        write_toast_lnX(ddir, rdir, rmeas, root, w, srcs, 'DC', 
                            df_string, msmask_dir, msmask_string, span, outputfile)
        ...
        write_toast_lnX(ddir, tdir, tmeas, root, w, srcs, 'A', 
                            df_string, msmask_dir, msmask_string, span, outputfile)
        # A ref
        outputfile = os.path.join(toastddir, fem_str.format('ref', 'lnA', w))
        write_toast_lnX(ddir, rdir, rmeas, root, w, srcs, 'A', 
                            df_string, msmask_dir, msmask_string, span, outputfile)
        print >>sys.stderr, 'done'

        # phi ref
        outputfile = os.path.join(toastddir, fem_str.format('ref', 'phi', w))
        write_toast_phi(ddir, rdir, rmeas, root, w, srcs, 'phi', 
                            df_string, msmask_dir, msmask_string, span, outputfile)
        # phi trg
        print >>sys.stderr, '  phi...',
        outputfile = os.path.join(toastddir, fem_str.format('trg', 'phi', w))
        rpath = os.path.join(ddir, rdir)  
        rfile = root + '_' + rmeas + '_wl{0}_s{1}_phi.npy'
        write_toast_phi(ddir, tdir, tmeas, root, w, srcs, 'phi', 
                            df_string, msmask_dir, msmask_string, span, outputfile, 
                            rpath, rfile)
        print >>sys.stderr, 'done'
...
def driver_toast_data(*args):
    ...
    srcs_exclude = pl.get_val('SRC_EXCLUDE_LIST')
    if srcs_exclude is not None:
        srcs_exclude = np.array([int(s) for s in srcs_exclude.split()])
    sample_data(ddir, tdir, rdir, tmeas, rmeas, root, s_nrow, s_ncol, wls, nrow, ncol, 
                ssmaskdir, musp_corr, span, ro=ro, co=ro, rs=rs, cs=cs,
                sro=sro, sco=sco, srs=srs, scs=scs, srcs_exclude=srcs_exclude)
