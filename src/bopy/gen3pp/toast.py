...
def sample_data(ddir, tdir, rdir, tmeas, rmeas, sname, srcs, wls, nrow, ncol, 
    ...
    for s in srcs:
        ...
        for w in wls:
            mask = np.load(os.path.join(ssmaskdir, ssmask_string.format(sname, w, s)))
            src_mask |= mask 
    ...
    toastddir = 'toastdata_{0}_{1}_{2}_{3}'.format(ro, co, rs, cs)
...
def driver_toast_data(*args):
    ...
    sname = pl.get_val('STUDY_NAME')
    srcs = np.array([int(s) for s in pl.get_val('SOURCES').split()])
    ...
    sample_data(ddir, tdir, rdir, tmeas, rmeas, sname, srcs, wls, nrow, ncol, 
